...
rcloud.notebook.starrer.list <- function(notebook)
{
  ...
  if(starcount > num_starrers) {
    ...
    for(user in all_users) {
      if(length(rcs.get(rcs.key(".notebook", notebook, "stars", user))) > 0 && !user %in% rcs.get(star.starrerlist.key(notebook)))
        rcs.set(star.starrerlist.key(notebook), c(rcs.get(star.starrerlist.key(notebook)), user ))
    }
  }
